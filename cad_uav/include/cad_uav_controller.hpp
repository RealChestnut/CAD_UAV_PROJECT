...
void attitude_controller()
{
  ...
  //ang_acl.x = (imu_ang_vel.x-prev_ang_vel.x)/delta_t.count();
...
void altitude_controller()
{
  ...
  if(!attitude_mode) F_xyzd.z = mass_system*(W2B_rot(2,0)*XYZ_ddot_desired.x
...
void wrench_allocation()
{
  ...
  //////////// force distribute ////////////
  F_xyzd_main.x = F_xyzd.x*allocation_factor(0);
  F_xyzd_main.y = F_xyzd.y*allocation_factor(0);
  F_xyzd_main.z = F_xyzd.z*allocation_factor(0); 
...
void PWM_signal_Generator()
{
  ...
  Force_prop.data[0]=F2;
  Force_prop.data[0]=F3;
  Force_prop.data[0]=F4;
