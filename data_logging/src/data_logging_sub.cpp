...
int main(int argc, char **argv)
{
	...
	ros::Subscriber servo_angle_log=nh.subscribe("joint_states_sub",1,servo_angle_callback,ros::TransportHints().tcpNoDelay());
	ros::Subscriber desired_servo_angle_log=nh.subscribe("goal_dynamixel_position_sub",1,desired_servo_angle_callback,ros::TransportHints().tcpNoDelay());
	...
	ros::Subscriber sbus_log=nh.subscribe("sbus_sub",1,sbus_callback,ros::TransportHints().tcpNoDelay());
	ros::Subscriber delta_t_log=nh.subscribe("delta_t",1,sampling_time_callback,ros::TransportHints().tcpNoDelay());
	ros::Subscriber battery_voltage_log=nh.subscribe("battery_voltage_sub",1,battery_voltage_callback,ros::TransportHints().tcpNoDelay());
